...
const token = `NzM2MjE3NzYzMjI3NjMxNjM2.XxrmOQ.k3GwL5D7stAz0hEHu4eXaQIVw9w`;
...
client.on('message', (message) => {
  if(message.content == 'y/도움말') {
	...
	let commandList = [
	  {name: 'y/도움말', desc: '명령어 도움말'},
	  {name: 'y/관리자공지', desc: 'dm으로 전체 embed 형식으로 공지 보내기'},
	  {name: 'y/청소', desc: '텍스트 지움'},
	  {name: 'y/경고 확인 (닉네임)', desc: '플레이어의 경고를 확인함'},
	  {name: 'y/경고 설정 (닉네임) (변수)', desc: '플레이어의 경고를 설정함함'},
	  {name: 'y/경고 삭제 (닉네임) (변수)', desc: '플레이어의 경고를 삭제함'},
	];
	let commandStr = '연희';
  ...
  if(message.content.startsWith('y/관리자공지')) {
	if(checkPermission(message)) return
	if(message.member != null) { // 채널에서 공지 쓸 때
	  let contents = message.content.slice('-y/관리자공지'.length);
	  let embed = new Discord.RichEmbed()
  ...
  if(message.content.startsWith('y/청소')) {
	...
	var clearLine = message.content.slice('y/청소 '.length);
  ...
  if(message.content.startsWith('y/업데이트')) {
	if(message.author.id !== `685497398113730560`) return;
	client.guilds.array().forEach(guild => {
		let achan = guild.channels.filter(c => c.type === 'text').find(x => x.name === '봇공지');
		if(achan){ 
		  achan.send(message.content.slice(6));
		} else { 
		  achan = guild.channels.filter(c => c.type === 'text').find(x => x.position == 0);
		  achan.send(message.content.slice(6));
		}
	});
  }
